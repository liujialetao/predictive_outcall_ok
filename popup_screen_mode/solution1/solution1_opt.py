    ...
    while(manage_customers.all_customers==[] or manage_customers.all_customers[-1].customer_id < customer_num):
        ...
        if next_customer!=None and next_customer.call_batch==now_call_batch_id:
            ...
            next_time_create_customer2 = next_customer.artificial_duration.start_time + \
                                         datetime.timedelta(seconds=(min(seconds_delay, next_customer.artificial_duration.duration.total_seconds())))
            print('seconds_delay:', seconds_delay, '\t 聊天时长：',next_customer.artificial_duration.duration.total_seconds())
        ...
        if next_customer!=None and next_customer.call_batch!=now_call_batch_id:
            ...
            if next_customer.call_batch not in trigger_call_batch:
                ...
                next_time_create_customer2 = next_customer.artificial_duration.start_time + \
                                             datetime.timedelta(seconds=(min(seconds_delay, next_customer.artificial_duration.duration.total_seconds())))
                print('seconds_delay:', seconds_delay, '\t 聊天时长：',
...
for call_num in [1,2,3,4]:  # ,4,5,6
    for seconds_delay in [7,14,21,28]:
        for max_wait_time in [40,50,60]:  # 20,25,30,35,40
            print('\n\n进度', 'call_num:', call_num, 'seconds_delay:', seconds_delay, 'max_wait_time:', max_wait_time)
            call_loss_ration, average_artificial_time_cal, busy_ration, total_idle_time, average_idle_time, average_dial_interval = simulation( customer_num, call_num, seconds_delay, max_wait_time)
...
df_filename = '../result/solution1_opt_test1_2.csv'
if not os.path.exists(df_filename):
    df.to_csv(df_filename)
else:
    df_old = pd.read_csv(df_filename, index_col=0)

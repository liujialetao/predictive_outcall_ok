...
def simulation(customer_num, call_num, seconds_delay, max_wait_time):
    ...
    while(manage_customers.all_customers==[] or manage_customers.all_customers[-1].customer_id < customer_num):
        ...
        if next_customer!=None and next_customer.call_batch==now_call_batch_id:
            ...
            # 步骤4：计算下次呼叫实际
            next_time_create_customer2 = avalable_arificial_time#next_customer.popup_screen_time[0] + datetime.timedelta(seconds=seconds_delay)
            call_flag = 1
        ...
        if next_customer!=None and next_customer.call_batch!=now_call_batch_id:
            ...
            if next_customer.call_batch not in trigger_call_batch:
                ...
                # 步骤4：计算下次呼叫实际
                next_time_create_customer2 = avalable_arificial_time #avalable_arificial_timenext_customer.popup_screen_time[0] + datetime.timedelta(seconds=seconds_delay)
                call_flag = 1
            # 该批次触发过拨号  不再拨号
            else:
                call_flag = 0
...
for call_num in [1,2,3,4]:  # ,4,5,6
    for seconds_delay in [7,14,21,28,35,42]:
...
df_filename = '../result/solution3_test_2.csv'
if not os.path.exists(df_filename):
    df.to_csv(df_filename)
else:
    df_old = pd.read_csv(df_filename, index_col=0)

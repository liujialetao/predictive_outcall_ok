...
class ManageArtificialSeats():
    ...
    def cal_total_monitor_and_artificial_time(treated_customers, successful_flag):
        ...
        return count_num, monitor_total_time, total_artificial_time
    ...
    def cal_busy_ration(manage_seats):
        ...
        def cal_average_parameter(row):
            ...
            计算均值
            df的列名：
            Index(['total_time', 'successful_count_num', 'successful_monitor_total_time',
       'successful_total_artificial_time', 'unsuccessful_count_num',
       'unsuccessful_monitor_total_time', 'unsuccessful_total_artificial_time',
       'count_num', 'monitor_time', 'total_artificial_time'],
      dtype='object')
            '''
            # 人工通话平均时长
            ...
            # 忙占比
            busy_ration1 = row.total_artificial_time/row.total_time
            busy_ration2 = (row.total_artificial_time+row.monitor_time)/row.total_time

            # 计算总共的空闲时间
            total_idle_time1 = row.total_time - row.total_artificial_time
            total_idle_time2 = row.total_time - row.total_artificial_time - row.monitor_time

            # 计算平均空闲时长
            average_idle_time1 = total_idle_time1/(row.successful_count_num-1)
            average_idle_time2 = total_idle_time2/(row.count_num-1)

            if average_idle_time2<0:
                print('有错')
            if len(artificial_seat.call_handled) >= 2:
                print('\t总客户聊天时长为{:.0f}秒'.format(row.total_artificial_time))
                print('\t总监控时长为{:.0f}秒'.format(row.monitor_time))
                print('\t人工通话平均时长为{:.2f}秒'.format(average_artificial_time_cal))

                print('不考虑监听时长时：')
                print('\t忙占比为：{:.2f}%'.format(busy_ration1*100))
                print('\t进线时长为：{:.2f}秒'.format(average_idle_time1))

                print('考虑监听时长时：')
                print('\t忙占比为：{:.2f}%'.format(busy_ration2*100))
                print('\t进线时长为：{:.2f}秒'.format(average_idle_time2))

                print('成功转接的人，被监听时间为：{:.2f}'.format(row.successful_monitor_total_time/row.successful_count_num),'采样值为7秒')
                print('未成功转接的人，被监听时间为：{:.2f}'.format(row.unsuccessful_monitor_total_time/row.unsuccessful_count_num), '无采样值')
                print('坐席人员，实际平均监听时间：{:.2f}'.format(row.monitor_time/row.count_num), '无采样值')
                print()
            return average_artificial_time_cal, busy_ration1, busy_ration2, average_idle_time1, average_idle_time2
        ...
        for artificial_seat in manage_seats.all_artificial_seats:
            ...
            successful_count_num, successful_monitor_total_time, successful_total_artificial_time = ManageArtificialSeats.cal_total_monitor_and_artificial_time(artificial_seat.call_handled, 'successful')
            # 统计未成功转人工坐席，总数量，总监控时间，总人工坐席时间
            unsuccessful_count_num, unsuccessful_monitor_total_time, unsuccessful_total_artificial_time = ManageArtificialSeats.cal_total_monitor_and_artificial_time(artificial_seat.call_handled, 'unsuccessful')

            count_num = successful_count_num + unsuccessful_count_num
            monitor_time = successful_monitor_total_time + unsuccessful_monitor_total_time
            total_artificial_time = successful_total_artificial_time + unsuccessful_total_artificial_time

            dic = {
                "total_time":total_time,
                "successful_count_num":successful_count_num,
                "successful_monitor_total_time":successful_monitor_total_time,
                "successful_total_artificial_time":successful_total_artificial_time,
                "unsuccessful_count_num":unsuccessful_count_num,
                "unsuccessful_monitor_total_time":unsuccessful_monitor_total_time,
                "unsuccessful_total_artificial_time":unsuccessful_total_artificial_time,
                "count_num":count_num,
                "monitor_time":monitor_time,
                "total_artificial_time":total_artificial_time,
            }

            df_new = pd.DataFrame([dic])
            df = df.append(df_new)
            print('坐席{}总时长为{:.0f}秒'.format(artificial_seat.id, total_time))
            cal_average_parameter(df_new.iloc[0])

        row_total = df.sum()
        print('综合{}个坐席看'.format(len(manage_seats.all_artificial_seats)))
        average_artificial_time_cal, busy_ration1, busy_ration2, average_idle_time1, average_idle_time2 = cal_average_parameter(
            row_total)
